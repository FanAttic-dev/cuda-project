...
{	
	...
	for (int dy = -1; dy <= 1; ++dy) {
		for (int dx = -1; dx <= 1; ++dx) {
			...
			if (shared_in_block[yy+1][xx+1] > 0)
				++inf_neighbours;
		}
	}
...
__global__ void make_iteration(const int* const contacts, const int* const in, const int n, const int iter, int* const out, int* const iter_block_infections)
{
	...
	if (threadIdx.x == 0) { // left
		...
		if (threadIdx.y == 0) // upper left
			shared_in_block[0][0] = (x == 0 || y == 0) ? 0 : in[idx_global - n - 1];
		else if (threadIdx.y == BLOCK_SIZE - 1) // bottom left
	...
	} else if (threadIdx.x == BLOCK_SIZE - 1) { // right
		shared_in_block[threadIdx.y+1][BLOCK_SIZE+1] = (x == n-1) ? 0 : in[idx_global + 1];

		if (threadIdx.y == 0) // upper right
			shared_in_block[0][BLOCK_SIZE+1] = (x == n-1 || y == 0) ? 0 : in[idx_global - n + 1];
		else if (threadIdx.y == BLOCK_SIZE - 1) // bottom right
			shared_in_block[BLOCK_SIZE+1][BLOCK_SIZE+1] = (x == n-1 || y == n-1) ? 0 : in[idx_global + n + 1];

	} else if (threadIdx.y == BLOCK_SIZE - 1) { // bottom
