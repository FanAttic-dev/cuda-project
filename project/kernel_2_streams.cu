	...
	if (house_in == 0) { // healthy
		...
		for (int dy = -1; dy <= 1; ++dy) {
			...
			for (int dx = -1; dx <= 1; ++dx) {
				// check bounds
				if ((x + dx >= 0) && (x + dx < n) && (in[row + (x + dx)] > 0))
					++infected_neighbours;
			}
		}
...
void solveGPU(const int* const contacts, int* const city, int* const infections, const int n, const int iters)
{
	...
	for (int iter = 0; iter < iters; ++iter) {
		make_iteration_inner<<<dim3(grid_size.x-2, grid_size.y-2), threads_per_block, 0, stream1>>>(in, out, contacts, infections, iter, n);
		make_iteration_corner<<<dim3(2, 2), threads_per_block, 0, stream1>>>(in, out, contacts, infections, iter, n, grid_size);
		make_iteration_border_y<<<dim3(2, grid_size.y-2), threads_per_block, 0, stream2>>>(in, out, contacts, infections, iter, n, grid_size);
		make_iteration_border_x<<<dim3(grid_size.x-2, 2), threads_per_block, 0, stream2>>>(in, out, contacts, infections, iter, n, grid_size);
